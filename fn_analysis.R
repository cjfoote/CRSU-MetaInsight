...
freq.df <- function(model,outcome,dataf,lstx) {
  net1 <- netmeta(TE, seTE, treat1, treat2, studlab, data = dataf, subset=NULL,
                  sm = outcome, level=0.95, level.comb=0.95,
                  comb.random=(model=="random"), comb.fixed = (model=="fixed"),reference.group =as.character(lstx[1]),
...
freq_wrap <- function(data, treat_list,model,outcome, CONBI) {
  ...
  net1<-freq.df(model,outcome ,d1,lstx) # NMA of all studies
...
groupforest.df <- function(d1, ntx, lstx, data_final, outcome) {
  ...
  if (outcome == "OR" | outcome =="RR" ){
    fplot <- forest(d1$TE, d1$seTE, slab = paste(data_final$Study.y), subset = order(d1$treat1, d1$treat2), ylim = c(1, nrow(d1) + 2*length(text_label) + 2),rows = lines, 
...
forest.df <- function(netresult,model,lstx) {
    fp<- forest(netresult,reference.group=as.character(lstx[1]), pooled=model)
return(fp)    
}
...
baye <- function(data,treat_list, model, outcome, CONBI) {
  ...
  progress$inc(0.4, detail="Rendering results")
  mtcRelEffects <- relative.effect(mtcResults,t1=treat_list2[1,2])  #Set "Placebo" as reference treatment
...
gemtctau <- function(results) {
  ...
  if (results$a=="random effect") {   #SD and its 2.5% and 97.5%
    sd_mean<- round(sumresults$summaries$statistics[results$ntx,1], digits = 2)
    sd_lowCI<-round(sumresults$summaries$quantiles[results$ntx,1], digits = 2)
    sd_highCI<-round(sumresults$summaries$quantiles[results$ntx,5], digits=2)
  }
  else {
    sd_mean =0
    sd_lowCI=0
