...
shinyUI(navbarPage(id="meta",
  ...
  tabPanel(id="home", "Home", 
     h2("MetaInsight (including Bayesian estimates) V3 **", tags$sup("Beta", style="color:#6CC0ED"), align= "left"),
     ...
     fluidRow(
       ...
       column(5, 
       br(),
       p(tags$strong("** New features updated on 25 March 2020 (V3) ** :")),
       actionLink("history_click", "Click here to view a full update history of MetaInsight"),
       p(tags$ul(tags$li( "We increased the memory on the 27th February 2020 to tackle the offloading problem experienced by the users 
       when analysing large size networks. If users still encounter frequent offloading problem, please feel free to contact the organiser.", style="color:#FF0000"))),
       p(tags$ul(tags$li("The binary and continuous app are now combined into one app. 
                         The users just need to select the outcome option at the 'Home' page based on the outcome in their study. "))),
       p(tags$ul(tags$li("The deviance plots of the Bayesian analysis on the 'Data analysis' tab - 
                         '3f. Deviance report' are now interactive. User can hover their curser on the data points to 
                         identify the corresponding study.")))
       )),
       br(),
         ...
         fluidRow( 
           ...
           column(9, tags$div(class="header", checked=NA,
              tags$p("Please click ", tags$a(href="http://www.nihrcrsu.org", "here ",
                                             target="_blank"), "for more information about the Complex Reviews Support Unit (CRSU)")
        ))))),
     ...
     mainPanel(
       ...
       tabsetPanel(
         tabPanel("1. Data summary", tabsetPanel(
            tabPanel("1a. Study Results", plotOutput("forestPlot", height = "1000px", width = "800px"), radioButtons('format_freq0', 'Document format', c('PDF', 'SVG'), inline = TRUE),  downloadButton('downloadStudy')),
            tabPanel("1b. Network Plot",
            column(6, plotOutput("netGraphStatic"), radioButtons('format_freq1', 'Document format', c('PDF', 'PNG'), inline = TRUE),
            ...
            column(6, plotOutput("netGraphUpdating"), radioButtons('format_freq2', 'Document format', c('PDF', 'PNG'), inline = TRUE),
         ...
         tabPanel("2. Frequentist network meta-analysis", tabsetPanel(
            tabPanel("2a. Forest Plot",
                ...
                column(6, plotOutput("SFPUpdatingComp", height = "400px", width = "400px"), textOutput("text5"), textOutput("ref3"), radioButtons('format_freq4', 'Document format', c('PDF', 'PNG'), inline = TRUE), downloadButton('downloadComp'))
                 ),
